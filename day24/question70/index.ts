/* Question : 70
        ...
        Explain % TIP : The let keyword providers block scope in JavaScript, which means variables declared with let inside a loop are only accessible within that loop. This helps avoid unintended interference with other parts of your code.
*/
...
function printNumbersWithLet(){
    ...
    // `i` is not accessible here, outside the loop, because it's defined with `let` (block scope.)
}
